...
from utils import awgn_channel, reparameterize, gaussian_kl_divergence, train_density_ratio_estimator, estimate_density_ratio, kl_log_uniform, sample_log_uniform_signed
import numpy as np
...
#------ CU-Encoder
        self.cu_enc = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=3, padding=1),  # Input: 3x32x32, Output: 8x32x32
            nn.ReLU(),
            nn.Conv2d(64, 32, kernel_size=3, padding=1),  # Output: 4x32x32
            nn.ReLU(),
            nn.MaxPool2d(2, 2),                        # Output: 4x16x16
            nn.Dropout(0.2),
            nn.Conv2d(32, 32, kernel_size=3, padding=1), # Output: 4x16x16
            nn.ReLU(),
            nn.MaxPool2d(2, 2),                        # Output: 4x8x8
            nn.Dropout(0.2),
            nn.Conv2d(32, 16, kernel_size=3, padding=1), # Output: 4x16x16
            nn.ReLU(),
            nn.MaxPool2d(2, 2),                        # Output: 4x8x8
            nn.Dropout(0.2)
        )
        self.fc1 = nn.Linear(16*4*4, n_c_h)
...
#------ CU-Decoder (auxiliary)
        self.cu_dec_first = nn.Sequential(
            ...
            nn.Linear(n_c_latent,128), nn.ReLU(), nn.Dropout(0.2)
        )
        self.cu_dec_first_out = nn.Linear(128, 1)

        self.cu_dec_second = nn.Sequential(
            nn.Linear(n_c_latent,n_c_latent), nn.ReLU(),
            nn.Linear(n_c_latent,128), nn.ReLU(),nn.Dropout(0.2)
        )
        self.cu_dec_second_out = nn.Linear(128, 10)
...
#------ SU1-Encoder
        ...
        self.su_dec_first_out = nn.Linear(128, 1)
...
#------ SU2-Encoder
        ...
        self.su_dec_second_out = nn.Linear(128, 10) 
...
#------ SU1-Encoder
        ...
        self.fs = nn.Linear(16*4*4, n_x_h)
        ...
        self.su_dec_ohnecu_out = nn.Linear(128, 1)
...
class MultiTaskMultiUserComm:
    ...
    def _loss_SU_first(self, s, targets1):
        ...
        alpha_squared = 1 / (x.pow(2)+1e-8)
        kl = kl_log_uniform(alpha_squared)
        ...
        loss_su1 = sre_sum + (0.0001)*(kl - dre_sum)
        return loss_su1
    ...
    def _loss_SU_second(self, s, targets2):
        ...
        dre_sum = torch.sum(dre)
        loss_su2 = sre2_sum + (0.0001)*(kl - dre_sum)
        return loss_su2
    ...
    def fit(self, first_dataset, second_dataset, first_test_dataset, second_test_dataset, batch_size=100,
        ...
        optimizer_su_first = torch.optim.Adam(self.specific_network_first.parameters(), lr= learning_rate) # for SUs we need to use SGD to ignore the DRE networks in updates
        optimizer_su_second = torch.optim.Adam(self.specific_network_second.parameters(), lr= learning_rate)
        optimizer_su_ohnecu = torch.optim.Adam(self.specific_network_ohnecu.parameters(), lr= learning_rate)
        ...
        for epoch_primal in range(n_epoch_primal):
            ...
            for batch, (S, targets1) in enumerate(first_loader):
                ...
                x_de_log_uniform_first = sample_log_uniform_signed(x.shape, low_exp=-6, high_exp=3, base=2)
                x_de_collect_first.append(x_de_log_uniform_first.detach().numpy())
            ...
            for batch, (S, targets2) in enumerate(second_loader):
                ...
                x_de_log_uniform_second = sample_log_uniform_signed(x2.shape, low_exp=-6, high_exp=3, base=2)
                x_de_collect_second.append(x_de_log_uniform_second.detach().numpy())
            ...
            for batch, (S, targets1) in enumerate(first_loader):
                ...
                x_de_log_uniform_ohnecu = sample_log_uniform_signed(x3.shape, low_exp=-6, high_exp=3, base=2)
                #x_de_log_uniform_ohnecu = sample_log_uniform_signed(x3.shape, low_exp=-4, high_exp=2, base=2, device=x3.device)
                x_de_collect_ohnecu.append(x_de_log_uniform_ohnecu.detach().numpy())
            ...
            self.dre_first_theta, self.dre_first_b, self.dre_first_X = train_density_ratio_estimator(x_nu_array_first, x_de_array_first, sample_size=4000)
            self.dre_second_theta, self.dre_second_b, self.dre_second_X = train_density_ratio_estimator(x_nu_array_second, x_de_array_second, sample_size=4000)
            self.dre_ohnecu_theta, self.dre_ohnecu_b, self.dre_ohnecu_X = train_density_ratio_estimator(x_nu_array_ohnecu, x_de_array_ohnecu, sample_size=4000)
